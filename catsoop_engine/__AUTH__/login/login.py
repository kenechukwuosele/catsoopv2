...
# import sys
# sys.path.append("/home/timilehin/.local/share/catsoop/courses/__USERS__")
# from register import generate_registration_form
...
def get_logged_in_user(context):
    ...
    # if the form has login information, we should try to authenticate
        ...
        if valid_uname and valid_pwd:
            ...
            role = login_info.get("role", "Student")
            info = {"username": uname, "name": name, "email": email, "role": role}
            session.update(info)
            session["course"] = context.get("cs_course", None)
            clear_session_vars(context, "login_message")
            info["cs_reload"] = True
...
def generate_registration_form(context):
    """
    Generate a registration form with role selection.
    """
    base = _get_base_url(context)
